...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    HWND_TOPMOST, IsWindowVisible, MB_ICONINFORMATION, MB_OK, MessageBoxW, PostMessageW,
    SM_CXSCREEN, SM_CYSCREEN, SWP_NOSIZE, SWP_SHOWWINDOW, SetWindowPos, WM_KEYDOWN, WM_KEYUP,
};
...
pub fn run_apps(paths: &[String], auto_login: bool, close_first: bool) -> Result<()> {
    ...
    if paths.is_empty() {
        return Err(anyhow!("启动失败，配置文件没有指定App路径"));
    }
    for path in paths {
        if path.is_empty() || !is_file_exists(&path) {
            return Err(anyhow!("启动失败，配置文件App路径不存在：{}", &path));
        }
    }
    if close_first {
